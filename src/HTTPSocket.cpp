...
void HTTPSocketRaw::BuildRequestString(Request &request, std::string &strRequest)
{
    ...
    for (std::vector<UrlOption>::iterator it = request.options.begin(); it != request.options.end(); ++it) {
        ...
        if(it->name == "Cookie")
            request.cookie = it->value;

        sprintf(buffer, "%s: %s\r\n", it->name.c_str(), it->value.c_str());
        strRequest += buffer;
    }

    //Add Content Length if Post Request
...
bool HTTPSocketRaw::Execute(Request &request, Response &response)
{
    ...
    //Parse for cookie
    std::smatch m;
    std::regex statusCodeRegex ("HTTP\\/1.[01] ([0-9]*) .*\r\n");
    if (std::regex_search(response.headers, m, statusCodeRegex)) {
        response.statusCode = m[1];
    }

    //Parse for cookie
    std::regex e ("Set-Cookie:(.*)\r\n");
    if (std::regex_search(response.headers, m, e)) {
        response.cookie = m[1];
    }
